...
/**
 * Implementation of IArrowIpcStreamIterator that converts VectorSchemaRoot batches to Arrow IPC
 * format messages for direct streaming access.
 *
 * <p>This class bridges between the existing ArrowStreamResult infrastructure and the ADBC
 * requirement for direct Arrow IPC message access. It performs on-the-fly serialization of Arrow
 * data to IPC wire format.
 *
 * <p>Key features:
 *
 * <ul>
 *   <li>Lazy serialization - IPC messages are created only when requested
 *   <li>Memory efficient - reuses buffers where possible
 *   <li>Progress tracking - monitors bytes read and batch count
 *   <li>Resource management - proper cleanup of Arrow resources
 * </ul>
 */
public class ArrowIpcStreamIterator implements IArrowIpcStreamIterator {

  private static final JdbcLogger LOGGER =
      JdbcLoggerFactory.getLogger(ArrowIpcStreamIterator.class);
  ...
  /**
   * Convenience constructor for ArrowStreamResult. This integrates with the existing Databricks
   * ArrowStreamResult infrastructure.
   */
  public static ArrowIpcStreamIterator fromArrowStreamResult(
      ArrowStreamResult arrowResult, BufferAllocator allocator, Schema schema) throws SQLException {

    try {
      // Use a simple stream of VectorSchemaRoot that will be created on-demand
      // This avoids the need to pre-create all batches
      Iterator<VectorSchemaRoot> vectorIterator =
          new java.util.Iterator<VectorSchemaRoot>() {
              ...
              if (!hasNextChecked) {
                try {
                  ...
                  if (nextRoot == null) {
                    ...
                    // Set empty data for now - actual implementation would populate from
                    // ArrowStreamResult
  ...
  public ByteBuffer next() {
    ...
    try {
      ...
      LOGGER.trace(
          "Serialized record batch to IPC: {} bytes, {} rows", messageSize, root.getRowCount());
  ...
  public void close() throws SQLException {
    if (closed.compareAndSet(false, true)) {
      try {
        ...
        LOGGER.debug(
            "Closed ArrowIPC stream iterator: {} batches, {} bytes",
            batchCount.get(),
            bytesRead.get());
  ...
  private ByteBuffer serializeSchemaToIpc(Schema schema) throws IOException {
    ...
    try (ArrowStreamWriter writer =
        new ArrowStreamWriter(
            VectorSchemaRoot.create(schema, allocator), null, Channels.newChannel(messageBuffer))) {
  ...
  private ByteBuffer serializeRecordBatchToIpc(VectorSchemaRoot root) throws IOException {
    ...
    try (ArrowStreamWriter writer =
        new ArrowStreamWriter(root, null, Channels.newChannel(messageBuffer))) {
