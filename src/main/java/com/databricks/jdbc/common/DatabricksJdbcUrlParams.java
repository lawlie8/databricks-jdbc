...
public enum DatabricksJdbcUrlParams {
  ...
  ADBC_BUFFER_SIZE(
      "AdbcBufferSize", "Buffer size for ArrowIPC message serialization (bytes)", "65536"),
  ADBC_PREFETCH_BATCHES(
      "AdbcPrefetchBatches", "Number of record batches to prefetch for ArrowIPC streaming", "2"),
  ADBC_ZERO_COPY_ENABLED(
      "AdbcZeroCopyEnabled", "Enable zero-copy optimizations for ArrowIPC streaming", "1"),
  ADBC_IPC_COMPRESSION(
      "AdbcIpcCompression", "Compression codec for ArrowIPC messages (none, lz4, zstd)", "none");
